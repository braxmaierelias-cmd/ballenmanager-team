...
/* ===== FarmManager V13 refinements ===== */
function fmCustomerFields(customerId){return fields.filter(f=>customerId?+f.customer_id===+customerId:!f.customer_id)}
function fmFieldCustomerName(f){const c=cust.find(x=>+x.id===+f.customer_id);return c?(c.name+(c.company?' · '+c.company:'')):'Nicht zugeordnet'}
function fmPopulateFieldCustomer(){const el=$('fieldCustomer');if(!el)return;const v=el.value;el.innerHTML='<option value="">Keinem Kunden zugeordnet</option>'+cust.map(c=>`<option value="${c.id}">${esc(c.name)}${c.company?' · '+esc(c.company):''}</option>`).join('');if(v)el.value=v}
function fmAllFieldsFiltered(){const q=($('fieldSearch')?.value||'').toLowerCase();return fields.filter(f=>[f.name,f.crop,f.notes,fmFieldCustomerName(f)].join(' ').toLowerCase().includes(q))}
function fmRenderAllFields(){const box=$('allFieldsList');if(!box)return;box.innerHTML=fmAllFieldsFiltered().map(f=>`<div class="field-master-row" data-field-focus="${f.id}"><div><b>${esc(f.name)}</b><span>${esc(fmFieldCustomerName(f))}</span><small>${f.hectares?Number(f.hectares).toFixed(2)+' ha':''}${f.crop?' · '+esc(f.crop):''}</small>${f.notes?`<p>${esc(f.notes)}</p>`:''}</div><div class="field-master-actions"><button class="secondary compact" data-field-edit="${f.id}">Bearbeiten</button><button class="secondary compact" data-field-delete="${f.id}">Löschen</button></div></div>`).join('')||'<p class="muted">Noch keine Felder angelegt.</p>';document.querySelectorAll('[data-field-focus]').forEach(r=>r.onclick=e=>{if(e.target.closest('button'))return;fmFocusField(+r.dataset.fieldFocus)});document.querySelectorAll('[data-field-edit]').forEach(b=>b.onclick=e=>{e.stopPropagation();fmEditField(+b.dataset.fieldEdit)});document.querySelectorAll('[data-field-delete]').forEach(b=>b.onclick=async e=>{e.stopPropagation();if(!confirm('Feld wirklich löschen?'))return;try{await remove('fields','id=eq.'+(+b.dataset.fieldDelete));await fmReloadFieldData()}catch(err){alert('Dieses Feld wird bereits verwendet und kann deshalb nicht gelöscht werden.')}});fmDrawAllFields()}
function fmDrawAllFields(){if(!fmFieldMap||!fmFieldDisplayLayer)return;fmFieldDisplayLayer.clearLayers();const bs=[];fields.forEach(f=>{const l=fmGeoLayer(f,{weight:2,fillOpacity:.13});if(!l)return;l.bindTooltip(`${esc(f.name)} · ${esc(fmFieldCustomerName(f))}`);l.addTo(fmFieldDisplayLayer);try{bs.push(l.getBounds())}catch(_){}});if(bs.length){let b=bs[0];for(let i=1;i<bs.length;i++)b.extend(bs[i]);fmFieldMap.fitBounds(b,{padding:[20,20],maxZoom:15})}}
const fmV12InitFieldMap=fmInitFieldMap;fmInitFieldMap=function(){fmV12InitFieldMap();if($('fieldMapProvider'))$('fieldMapProvider').textContent='Karte · Google Maps nach API-Key';fmDrawAllFields()}
function fmStartNewField(){fmPendingGeometry=null;$('fieldEditId').value='';$('fieldName').value='';$('fieldHectares').value='';$('fieldCrop').value='';$('fieldNotes').value='';$('fieldCustomer').value='';$('fieldEditorTitle').textContent='Neues Feld';$('fieldEditor').hidden=false;if(fmFieldDrawLayer)fmFieldDrawLayer.clearLayers();alert('Zeichne jetzt die Feldgrenze mit dem Polygon-Werkzeug oben links auf der Karte ein.')}
$('newFieldButton')?.addEventListener('click',fmStartNewField);$('fieldSearch')?.addEventListener('input',fmRenderAllFields);document.querySelector('nav button[data-page="felder"]')?.addEventListener('click',()=>setTimeout(()=>{fmPopulateFieldCustomer();fmInitFieldMap();fmRenderAllFields()},60));
fmSaveField=async function(){const customer_id=+$('fieldCustomer').value||null,name=$('fieldName').value.trim(),id=+$('fieldEditId').value||0;if(!name)return alert('Bitte Feldname eingeben.');if(!fmPendingGeometry)return alert('Bitte Feld auf der Karte einzeichnen.');const obj={customer_id,name,hectares:+$('fieldHectares').value||null,crop:$('fieldCrop').value.trim()||null,notes:$('fieldNotes').value.trim()||null,geometry:fmPendingGeometry,updated_at:new Date().toISOString()};try{if(id)await update('fields',obj,'id=eq.'+id);else await insert('fields',{...obj,created_by:me.id},false);fmCancelFieldEditor();await fmReloadFieldData();fmRenderAllFields()}catch(e){alert('Feld konnte nicht gespeichert werden: '+e.message)}}
const saveFieldClone=$('saveField')?.cloneNode(true);if(saveFieldClone){$('saveField').replaceWith(saveFieldClone);saveFieldClone.addEventListener('click',fmSaveField)}
const fmV12EditField=fmEditField;fmEditField=function(id){fmV12EditField(id);$('fieldEditorTitle').textContent='Feld bearbeiten'}
const fmV12RenderCustomers=renderCustomers;renderCustomers=function(){fmV12RenderCustomers();document.querySelectorAll('[data-edit-c]').forEach(btn=>{const id=+btn.dataset.editC,card=btn.closest('.customer');if(!card||card.querySelector('.customer-field-summary'))return;const list=fields.filter(f=>+f.customer_id===id),d=document.createElement('div');d.className='customer-field-summary';d.innerHTML=`<b>Felder:</b> ${list.length?list.map(f=>`<span>${esc(f.name)}</span>`).join(' '):'<small>Keine Felder zugeordnet</small>'}`;card.appendChild(d)})}
function fmCompletedOrderEditButton(){document.querySelectorAll('#completedOrders .order').forEach(card=>{if(card.querySelector('.edit-completed-order'))return;const m=(card.querySelector('h4')?.textContent||'').match(/#(\d+)/);if(!m)return;const b=document.createElement('button');b.className='secondary edit-completed-order';b.textContent='Bearbeiten';b.onclick=()=>fmOpenCompletedOrderEdit(+m[1]);card.appendChild(b)})}
function fmOpenCompletedOrderEdit(id){const o=orders.find(x=>+x.id===+id);if(!o)return;$('completedOrderEditId').value=o.id;$('completedOrderEditTitle').textContent=`Auftrag #${o.id} bearbeiten`;$('completedOrderCustomer').innerHTML=cust.map(c=>`<option value="${c.id}">${esc(c.name)}${c.company?' · '+esc(c.company):''}</option>`).join('');$('completedOrderAssigned').innerHTML='<option value="">Nicht zugeordnet</option>'+profiles.map(p=>`<option value="${p.id}">${esc(p.display_name||p.email)}</option>`).join('');$('completedOrderCustomer').value=o.customer_id||'';$('completedOrderStatus').value=o.status||'abgeschlossen';$('completedOrderAssigned').value=o.assigned_to||'';$('completedOrderPayment').value=o.payment_status||'offen';$('completedOrderDate').value=o.delivery_date||'';$('completedOrderNotes').value=o.notes||'';fmRenderCompletedOrderFields(o);$('completedOrderEditor').hidden=false;$('completedOrderEditor').scrollIntoView({behavior:'smooth',block:'start'})}
function fmRenderCompletedOrderFields(o){const list=fields.filter(f=>+f.customer_id===+o.customer_id),sel=new Set(fmOrderFieldIds(o.id));$('completedOrderFields').innerHTML=list.map(f=>`<label class="field-check"><input type="checkbox" value="${f.id}" ${sel.has(+f.id)?'checked':''}><span><b>${esc(f.name)}</b><small>${f.hectares?Number(f.hectares).toFixed(2)+' ha':''}</small></span></label>`).join('')||'<p class="muted">Keine Felder für diesen Kunden.</p>'}
$('completedOrderCustomer')?.addEventListener('change',()=>{const o=orders.find(x=>+x.id===+$('completedOrderEditId').value);if(o)fmRenderCompletedOrderFields({...o,customer_id:+$('completedOrderCustomer').value})});$('cancelCompletedOrderEdit')?.addEventListener('click',()=>{$('completedOrderEditor').hidden=true});$('saveCompletedOrderEdit')?.addEventListener('click',async()=>{const id=+$('completedOrderEditId').value,o=orders.find(x=>+x.id===id);if(!o)return;try{await update('orders',{customer_id:+$('completedOrderCustomer').value||o.customer_id,status:$('completedOrderStatus').value,assigned_to:$('completedOrderAssigned').value||null,payment_status:$('completedOrderPayment').value,delivery_date:$('completedOrderDate').value||null,notes:$('completedOrderNotes').value.trim()||null,updated_at:new Date().toISOString()},'id=eq.'+id);const selected=[...document.querySelectorAll('#completedOrderFields input:checked')].map(x=>+x.value);await remove('order_fields','order_id=eq.'+id);if(selected.length)await insert('order_fields',selected.map(field_id=>({order_id:id,field_id,created_by:me.id,added_during_work:false})),false);$('completedOrderEditor').hidden=true;await load()}catch(e){alert('Auftrag konnte nicht gespeichert werden: '+e.message)}})
const fmV12RenderOrders=renderOrders;renderOrders=function(){fmV12RenderOrders();setTimeout(fmCompletedOrderEditButton,0)}
function fmLocalInput(iso){if(!iso)return'';const d=new Date(iso),off=d.getTimezoneOffset();return new Date(d.getTime()-off*60000).toISOString().slice(0,16)}
function fmPopulateWorkEditOrders(){const cid=+$('workEditCustomer').value||0;$('workEditOrder').innerHTML=orders.filter(o=>+o.customer_id===cid).map(o=>`<option value="${o.id}">#${o.id} · ${esc(o.order_type)} · ${esc(L[o.status]||o.status)}</option>`).join('')}
function fmOpenWorkEdit(id){const s=workSessions.find(x=>+x.id===+id);if(!s||s.user_id!==me.id)return;$('workEditId').value=s.id;$('workEditStart').value=fmLocalInput(s.started_at);$('workEditEnd').value=fmLocalInput(s.ended_at);$('workEditPause').value=Math.round((+s.pause_seconds||0)/60);$('workEditMachine').innerHTML='<option value="">Keine Maschine</option>'+machines.map(m=>`<option value="${m.id}">${esc(m.name)}</option>`).join('');$('workEditCustomer').innerHTML=cust.map(c=>`<option value="${c.id}">${esc(c.name)}${c.company?' · '+esc(c.company):''}</option>`).join('');$('workEditCustomer').value=s.customer_id;fmPopulateWorkEditOrders();$('workEditOrder').value=s.order_id;$('workEditMachine').value=s.machine_id||'';$('workEditType').value=s.work_type;$('workEditNotes').value=s.notes||'';$('workEditCard').hidden=false;$('workEditCard').scrollIntoView({behavior:'smooth',block:'start'})}
$('workEditCustomer')?.addEventListener('change',fmPopulateWorkEditOrders);$('cancelWorkEdit')?.addEventListener('click',()=>{$('workEditCard').hidden=true});$('saveWorkEdit')?.addEventListener('click',async()=>{const id=+$('workEditId').value,s=workSessions.find(x=>+x.id===id);if(!s||s.user_id!==me.id)return;const start=new Date($('workEditStart').value),end=new Date($('workEditEnd').value);if(isNaN(start)||isNaN(end)||end<=start)return alert('Start- und Endzeit prüfen.');const pause=Math.max(0,+$('workEditPause').value||0)*60,work=Math.max(0,Math.floor((end-start)/1000)-pause);try{await update('work_sessions',{started_at:start.toISOString(),ended_at:end.toISOString(),pause_seconds:pause,work_seconds:work,machine_id:+$('workEditMachine').value||null,customer_id:+$('workEditCustomer').value,order_id:+$('workEditOrder').value,work_type:$('workEditType').value,notes:$('workEditNotes').value.trim()||null,updated_at:new Date().toISOString()},'id=eq.'+id);$('workEditCard').hidden=true;await fmLoadFeatures();alert('Arbeitstag wurde aktualisiert.')}catch(e){alert('Arbeitstag konnte nicht gespeichert werden: '+e.message)}})
const fmV12RenderWorkHistory=fmRenderWorkHistory;fmRenderWorkHistory=function(){fmV12RenderWorkHistory();const list=(myRole==='admin'?workSessions:workSessions.filter(x=>x.user_id===me.id)).slice(0,60),rows=[...document.querySelectorAll('#workHistory .work-row')];rows.forEach((row,i)=>{const s=list[i];if(!s||s.user_id!==me.id||s.status!=='completed'||row.querySelector('.edit-work-row'))return;const b=document.createElement('button');b.className='secondary compact edit-work-row';b.textContent='Bearbeiten / ergänzen';b.onclick=()=>fmOpenWorkEdit(s.id);row.querySelector('.work-row-right')?.appendChild(b)})}
const fmV12ReloadFieldData=fmReloadFieldData;fmReloadFieldData=async function(){await fmV12ReloadFieldData();fmRenderAllFields();renderCustomers();renderOrders()};setTimeout(()=>{fmRenderAllFields();fmCompletedOrderEditButton()},900)
